...
| 2 | `menge` | INTEGER | ❌ | Lagerbestand (Stückzahl) |
| 3 | `ort` | TEXT | ❌ | Lagerort (z.B. Regal, Raum) |
| 4 | `fach` | TEXT | ❌ | Lagerfach innerhalb des Orts |
| 5 | `beschreibung` | TEXT | ❌ | Freitextbeschreibung des Artikels |
| 6 | `bildPfad` | TEXT | ❌ | Lokaler Dateipfad zum Originalbild |
| 7 | `thumbnailPfad` | TEXT | ❌ | Lokaler Dateipfad zum Thumbnail |
| 8 | `thumbnailEtag` | TEXT | ❌ | ETag des Thumbnails (Cache-Validierung) |
| 9 | `erstelltAm` | TEXT | ❌ | ISO 8601 Erstellungszeitpunkt |
| 10 | `aktualisiertAm` | TEXT | ❌ | ISO 8601 letzter Änderungszeitpunkt |
| 11 | `remoteBildPfad` | TEXT | ❌ | Dateiname des Bildes auf PocketBase-Server |
| 12 | `uuid` | TEXT | ✅ | Geräteübergreifende eindeutige ID (UUID v4) |
| 13 | `updated_at` | INTEGER | ✅ (default 0) | Letzter Änderungszeitpunkt in **Millisekunden** (Unix) |
| 14 | `deleted` | INTEGER | ✅ (default 0) | Soft-Delete Flag: `0` = aktiv, `1` = gelöscht |
| 15 | `etag` | TEXT | ❌ | ETag für HTTP-Cache-Validierung |
| 16 | `remote_path` | TEXT | ❌ | **PocketBase Record-ID** (Fremdschlüssel zum Server) |
| 17 | `device_id` | TEXT | ❌ | Geräte-ID des erstellenden Geräts |
| 18 | `kategorie` | TEXT | ❌ | Artikelkategorie |
...
│ │
│ 2. LOCAL → REMOTE (Push) │
│ ├── WHERE updated_at > last_sync AND deleted = 0 │
│ │ └── CREATE oder UPDATE auf PocketBase │
│ └── WHERE updated_at > last_sync AND deleted = 1 │
│ └── DELETE auf PocketBase (Hard-Delete) │
│ │
│ 3. REMOTE → LOCAL (Pull) │
│ ├── Neue/geänderte Records von PocketBase holen │
│ └── Lokal einfügen oder aktualisieren │
│ │
...
│ ├── JA → Bild herunterladen → bildPfad setzen │
...
## ✅ 7. Verifikation (Stand: 2026-03-19)
...
*Dokumentation automatisch erstellt auf Basis von DB-Analyse – Lager App Projekt*