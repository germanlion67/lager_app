...
class DokumenteButtonState extends State<DokumenteButton> {
  ...
  // FIX 9: Kein late nötig — Singleton direkt initialisieren
  ...
  void initState() {
    super.initState();
    // FIX 7: Service starten damit Status nicht immer unknown bleibt
  ...
  Future<void> _loadCount() async {
    ...
    // FIX 2: mounted-Guard nach jedem await
    if (!mounted) return;
    ...
    try {
      ...
      // FIX 2: mounted-Guard nach await
      if (!mounted) return;
      ...
      // FIX 2: mounted-Guard im catch-Block
      if (!mounted) return;
      setState(() {
        _count = null;
        _dirExists = false;
  ...
  void _showDokumenteSheet() {
    showModalBottomSheet(
      ...
      builder: (ctx) {
        ...
        // FIX 3: Kommentar zur Klarheit — Variable korrekt außerhalb
        //        des builder-Callbacks deklariert, wird nicht bei
        //        jedem Rebuild zurückgesetzt.
        bool hasScheduledInitialLoad = false;

        return StatefulBuilder(
          builder: (contextSB, setStateSB) {
            // messenger vor async-Gap cachen — korrekt beibehalten
            final ScaffoldMessengerState? messenger =
                ScaffoldMessenger.maybeOf(contextSB);

            Future<void> loadFiles() async {
              debugPrint(
                'DokumenteSheet: loadFiles() gestartet '
                'für artikel ${widget.artikelId}',
              );
              unawaited(
                AppLogService().log(
                  'Dokumente: loadFiles() gestartet '
                  'für artikel ${widget.artikelId}',
                ),
              );
              ...
              try {
                ...
                unawaited(
                  AppLogService().log(
                    'Dokumente: creds geladen: ${creds != null} '
                    'für artikel ${widget.artikelId}',
                  ),
                );
                ...
                debugPrint(
                  'DokumenteSheet: Fetched ${fetched.length} '
                  'dokumente for artikel ${widget.artikelId}',
                );
                unawaited(
                  AppLogService().log(
                ...
                if (contextSB.mounted) {
                  try {
                    ...
                    messenger?.showSnackBar(
                      SnackBar(
                        content: Text('Fehler beim Laden der Dokumente: $e'),
                      ),
                    );
                  } catch (_) {}
                }
                        ...
                        PopupMenuButton<String>(
                          ...
                          onSelected: (v) {
                            if (v == 'name') {
                              ...
                              setStateSB(() {
                                files = sortFilesByTypeThenName(files);
                              });
                            }
                          },
                          itemBuilder: (_) => const [
                                ...
                                : ListView.separated(
                                    ...
                                    itemBuilder: (ctx, i) {
                                      ...
                                      if ([
                                        ...
                                        // FIX 5: Auth-Header für Nextcloud-Bilder
                                        final authHeader = currentConfig != null
                                            ? 'Basic ${base64Encode(
                                                utf8.encode(
                                      ...
                                      return Card(
                                        ...
                                        child: ListTile(
                                          ...
                                          onTap: () async {
                                            ...
                                            unawaited(
                                              AppLogService().log(
                                                'Dokumente: Download gestartet '
                                                'für $file '
                                                '(artikel ${widget.artikelId})',
                                              ),
                                            );

                                            // FIX 6: currentConfig wiederverwenden
                                            //         statt Credentials erneut lesen
                                            final cfg = currentConfig;
                                            if (cfg == null ||
                                                widget.artikelId == null) {
                                              setStateSB(() {
                                                downloading.remove(i);
                                              });
                                              unawaited(
                                                AppLogService().log(
                                                  'Dokumente: Abbruch Download '
                                                  '- keine Config oder ArtikelId',
                                                ),
                                              );
                                              return;
                                            }

                                            // FIX 1: contextSB.mounted statt
                                            //         mounted für BottomSheet-Context
                                            if (!contextSB.mounted) {
                                              setStateSB(() {
                                                downloading.remove(i);
                                              });
                                            ...
                                            try {
                                              await clientTap.downloadFile(
                                                '${widget.artikelId}/$file',
                                                tempPath,
                                              );
                                              unawaited(
                                                AppLogService().log(
                                                  'Dokumente: Download erfolgreich '
                                                  'für $file -> $tempPath',
                                                ),
                                              );

                                              // FIX 1: contextSB.mounted prüfen
                                              if (!contextSB.mounted) return;
                                              ...
                                              if (result.type ==
                                                ...
                                                unawaited(
                                                  AppLogService().log(
                                                    'Dokumente: Datei geöffnet: $file',
                                                  ),
                                                );
                                                // FIX 1: messenger statt
